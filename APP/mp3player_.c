...
//仅支持两层文件夹
u16 totdirnum; 		//音乐文件总数
u16 curindex2;		//当前索引
u16 *mp3dirindextbl;	//音乐dir索引表 

void mp3_next_dir(void)
{
	u8 res;
...
void mp3_play(void)
{
	u8 res;
 	u8 key;				//键值		  
 	u16 temp;
	gpath=mymalloc(SRAMIN,1024);				//为路径分配内存
	strcpy((char*)gpath, (const char*)MUSIC_DIR);
 	while(f_opendir(&mp3dir,MUSIC_DIR))//打开图片文件夹
 	{	    
	...
	totmp3num=mp3_get_tnum((u8*)MUSIC_DIR); //得到总有效文件数
	totdirnum=mp3_get_tnum_dir((u8*)MUSIC_DIR); //得到总有效文件数
	...
	mp3dirindextbl=mymalloc(SRAMIN,2*totdirnum);				//申请2*totdirnum个字节的内存,用于存放音乐文件索引
	...
	//记录索引
    res=f_opendir(&mp3dir,MUSIC_DIR); //打开目录
	if(res==FR_OK)
	{
		curindex=0;//当前索引为0
		curindex2=0;//当前索引为0
		while(1)//全部查询一遍
		{
			...
			if (mp3fileinfo.fattrib & AM_DIR)
			{
				mp3indextbl[curindex2]=temp;//记录索引
				curindex2++;
			}
		} 
	}   
   	curindex=0;											//从0开始显示
	curindex2=0;											//从0开始显示
   	res=f_opendir(&mp3dir,(const TCHAR*)MUSIC_DIR); 	//打开目录
