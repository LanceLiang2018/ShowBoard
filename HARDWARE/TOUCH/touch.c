...
#define SAVE_ADDR_BASE 0X8070000
//保存校准参数										    
void TP_Save_Adjdata(void)
{
    ...
    //AT24CXX_WriteLenByte(SAVE_ADDR_BASE,temp,4);   
		...
		STMFLASH_Write(SAVE_ADDR_BASE, (u16*)&temp, 4);
	temp=tp_dev.yfac*100000000;//保存y校正因素    
    //AT24CXX_WriteLenByte(SAVE_ADDR_BASE+4,temp,4);
		//W25QXX_Write((u8*)&temp, SAVE_ADDR_BASE+4, 4);
	STMFLASH_Write(SAVE_ADDR_BASE+4, (u16*)&temp, 4);
	//保存x偏移量
    //AT24CXX_WriteLenByte(SAVE_ADDR_BASE+8,tp_dev.xoff,2);		    
		//W25QXX_Write((u8*)&tp_dev.xoff, SAVE_ADDR_BASE+8, 2);
	STMFLASH_Write(SAVE_ADDR_BASE+8, (u16*)&tp_dev.xoff, 2);
	//保存y偏移量
		//AT24CXX_WriteLenByte(SAVE_ADDR_BASE+10,tp_dev.yoff,2);	
		//W25QXX_Write((u8*)&tp_dev.yoff, SAVE_ADDR_BASE+10, 2);
	STMFLASH_Write(SAVE_ADDR_BASE+10, (u16*)&tp_dev.yoff, 2);
	//保存触屏类型
	//AT24CXX_WriteOneByte(SAVE_ADDR_BASE+12,tp_dev.touchtype);	
	//W25QXX_Write((u8*)&tp_dev.touchtype, SAVE_ADDR_BASE+12, 1);
	STMFLASH_Write(SAVE_ADDR_BASE+12, (u16*)&tp_dev.touchtype, 2);
	temp=0X0A;//标记校准过了
	//AT24CXX_WriteOneByte(SAVE_ADDR_BASE+13,temp); 
	//W25QXX_Write((u8*)&tp_dev.touchtype, SAVE_ADDR_BASE+13, 1);
	STMFLASH_Write(SAVE_ADDR_BASE+14, (u16*)&temp, 2);
	
	/*tp_dev.adjust = TP_Adjust;
	tp_dev.init = TP_Init;
	tp_dev.scan = TP_Scan;
	tp_dev.check = 0x0A;
	STMFLASH_Write(SAVE_ADDR_BASE, (u16*)&tp_dev, sizeof(tp_dev));*/
}
//得到保存在EEPROM里面的校准值
//返回值：1，成功获取数据
//        0，获取失败，要重新校准
...
{					  
	...
	//W25QXX_Read((u8*)&tempfac, SAVE_ADDR_BASE+13, 1);
	STMFLASH_Read(SAVE_ADDR_BASE+14, (u16*)&tempfac, 2);
	/*STMFLASH_Read(SAVE_ADDR_BASE, (u16*)&tp_dev, sizeof(tp_dev));
	tp_dev.adjust = TP_Adjust;
	tp_dev.init = TP_Init;
	tp_dev.scan = TP_Scan;*/
	//tempfac = tp_dev.check;
	if(tempfac==0X0A)//触摸屏已经校准过了			   
	{    												 
		//tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE,4);		   
		//W25QXX_Read((u8*)&tempfac, SAVE_ADDR_BASE, 4);
		STMFLASH_Read(SAVE_ADDR_BASE, (u16*)&tempfac, 4);
		tp_dev.xfac=(float)tempfac/100000000;//得到x校准参数
		//tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+4,4);			          
		//W25QXX_Read((u8*)&tempfac, SAVE_ADDR_BASE+4, 4);
		STMFLASH_Read(SAVE_ADDR_BASE+4, (u16*)&tempfac, 4);
		tp_dev.yfac=(float)tempfac/100000000;//得到y校准参数
	    //得到x偏移量
		//tp_dev.xoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+8,2);			   	  
		//W25QXX_Read((u8*)&tp_dev.xoff, SAVE_ADDR_BASE+8, 2);
		STMFLASH_Read(SAVE_ADDR_BASE+8, (u16*)&tp_dev.xoff, 2);
 	    //得到y偏移量
		//tp_dev.yoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);				 	  
		//W25QXX_Read((u8*)&tp_dev.yoff, SAVE_ADDR_BASE+10, 2);
		STMFLASH_Read(SAVE_ADDR_BASE+10, (u16*)&tp_dev.yoff, 2);
 		//tp_dev.touchtype=AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12);//读取触屏类型标记
		//W25QXX_Read((u8*)&tp_dev.touchtype, SAVE_ADDR_BASE+12, 1);
		STMFLASH_Read(SAVE_ADDR_BASE+12, (u16*)&tp_dev.touchtype, 2);
...
{			    		   
  ...
  GPIO_SetBits(GPIOF,GPIO_Pin_9);//P9 上拉
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;	// PF8.PF10端口配置
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //	上拉输入
 	GPIO_Init(GPIOF, &GPIO_InitStructure);

 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;	// PF8.PF10端口配置
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;	// PPF10端口配置
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //	上拉输入
 	GPIO_Init(GPIOF, &GPIO_InitStructure);*/
...
OS_STK TP_TASK_STK[TP_STK_SIZE];
void TP_task(void *pdata)
{
	...
	while (1)
	{
		...
		if (res)
		{
			...
			{
				POINT_COLOR = RED;
				//LCD_DrawBigPoint(tp_dev.x, tp_dev.y);
				res = GUI_Click(tp_dev.x, tp_dev.y);
				if (res)
				{
					while (!PEN)
						delay_ms(15);
					GUI_Clicked(&g_obj[res-1]);
				}
			}
		}
		delay_ms(15);
