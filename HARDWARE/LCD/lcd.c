...
{
	LCD_Fast_DrawPoint(x,y,RED);//中心点 
	LCD_Fast_DrawPoint(x+1,y,RED);
	LCD_Fast_DrawPoint(x,y+1,RED);
	LCD_Fast_DrawPoint(x+1,y+1,RED);	 	  	
}
//SSD1963 背光设置
//pwm:背光等级,0~100.越大越亮.
...
{          
 	...
 	}else
	{
		...
		for(i=sy;i<=ey;i++)
		{
			...
			LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen-1;j++)LCD->LCD_RAM=color;	//显示颜色 	    
		}
	}	 
}  
//在指定区域内填充指定颜色块			 
...
void LCD_PWM_Set(u16 light)
{
	TIM_SetCompare3(TIM3, light);
}
